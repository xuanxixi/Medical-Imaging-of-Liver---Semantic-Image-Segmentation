    ...
    with tqdm(total=epoch_size, desc=f'Epoch {epoch + 1}/{Epoch}', postfix=dict, mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            pbar.set_postfix(**{'total_loss': total_loss / (iteration + 1),
                                ...
                                'lr': get_lr(optimizer)})
            pbar.update(1)
            log_writer.add_scalar('Loss/train', float(total_loss / (epoch_size + 1)), epoch)
            log_writer.add_scalar('f_score/train', float(total_f_score / (epoch_size + 1)), epoch)
    print('Finish Validation')
    print('Epoch:' + str(epoch + 1) + '/' + str(Epoch))
    print('Total Loss: %.4f' % (total_loss / (epoch_size + 1)))
    print('Saving state, iter:', str(epoch + 1))
    torch.save(model.state_dict(), 'logs/U-net/Epoch%d-Total_Loss%.4f.pth' % ((epoch + 1), total_loss / (epoch_size + 1)))
...
if __name__ == "__main__":
    ...
    if True:
        ...
        train_dataset = \
            (train_lines, inputs_size, NUM_CLASSES, True, dataset_path)
